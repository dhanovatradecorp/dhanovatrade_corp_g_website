...
const beautyPath = path.resolve(process.argv[2] ??
    process.env.BEAUTY_CATALOG_PATH ??
    path.join(downloads, "Beauty_Products_Catalogue_300 (1).xlsx"));
const cadburyPath = path.resolve(process.argv[3] ??
...
const beautyProducts = beautyRows.slice(3).flatMap((row) => {
    ...
    if (!id || !brand || !productName || mrp <= 0 || wholesale <= 0)
        return [];
...
const cadburyProducts = cadburyRows.slice(3).flatMap((row) => {
    ...
    if (!id || !productName || mrp <= 0 || wholesale <= 0)
        return [];
...
try {
    ...
    await Promise.all([...slugsBySource].map(([source, slugs]) => Product.updateMany({ "specifications.source": source, slug: { $nin: slugs } }, { $set: { isActive: false } })));
