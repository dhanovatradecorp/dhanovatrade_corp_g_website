...
const DEFAULT_PATH = path.join(
  homedir(),
  "Downloads",
  "staples_home_decor_catalog_1000.json",
);
const cataloguePath = path.resolve(
  process.argv[2] ??
    process.env.STAPLES_HOME_DECOR_CATALOG_PATH ??
    DEFAULT_PATH,
);
const categoryMap = {
...
const rows = JSON.parse(rawCatalogue);
...
const products = rows.map((row) => {
  ...
  const compareAtPrice =
    row.mrp > row.suggestedSellingPrice ? row.mrp : undefined;
  ...
  return {
    ...
    specifications: new Map([
      ...
      [
        "imageStatus",
        "Related category image used because the supplied catalogue image is a grey placeholder",
      ],
...
try {
  ...
  console.log(
    `Staples/Home/Decor import: ${result.upsertedCount} inserted, ${result.modifiedCount} updated.`,
  );
  console.log(
    sectionCounts.map((item) => `${item._id}: ${item.count}`).join("; "),
  );
