...
async function sourceFetch(path) {
    ...
    if (!response.ok)
        throw new Error(`Source catalog request failed (${response.status}): ${path}`);
...
function money(prices, field) {
    return Number(prices[field] || 0) / 10 ** prices.currency_minor_unit;
}
function packLabel(product, variation) {
    const termLookup = new Map(product.attributes.flatMap((attribute) => (attribute.terms ?? []).map((term) => [term.slug, term.name])));
    const selected = variation?.attributes
...
const sourceProducts = [
    ...(await sourceFetch("/products?per_page=100&page=1")),
    ...(await sourceFetch("/products?per_page=100&page=2")),
];
const sourceVariants = (await mapWithConcurrency(sourceProducts, 6, async (product) => {
    const variationRows = product.variations.length
        ? await mapWithConcurrency(product.variations, 4, async (reference) => ({
            reference,
            product: await sourceFetch(`/products/${reference.id}`),
        }))
    ...
    return variationRows.map(({ reference, product: pricedProduct }) => {
        ...
        const regularPrice = money(pricedProduct.prices, "regular_price") ||
            money(product.prices, "regular_price");
        const sourceCategories = product.categories.map((category) => category.name);
        const subcategory = sourceCategories.find((name) => !["Best Sellers", "Gift Hampers"].includes(name)) ??
            sourceCategories[0] ??
            "Dry Fruits";
...
const standardPacks = [250, 500, 750, 1000];
const standardLabel = (grams) => grams === 1000 ? "1 kg" : `${grams} g`;
...
const imported = Array.from(groupedVariants.values()).flatMap((variants) => {
    const weighted = variants
        ...
        .filter((entry) => entry.grams !== null && entry.product.price > 0);
    const sourceCategories = variants[0]?.tags ?? [];
    const excluded = sourceCategories.includes("Gift Hampers") ||
        /gift|diwali|wedding|shaadi|hamper/i.test(variants[0]?.name ?? "");
    if (!weighted.length || excluded)
        return [];
    return standardPacks.map((grams) => {
        const exact = weighted.find((entry) => entry.grams === grams);
        const reference = exact ??
            [...weighted].sort((a, b) => Math.abs(a.grams - grams) - Math.abs(b.grams - grams))[0];
        const price = exact
            ? exact.product.price
            : Math.max(1, Math.round((reference.product.price / reference.grams) * grams));
        const compareAtPrice = reference.product.compareAtPrice
            ? exact
                ? reference.product.compareAtPrice
                : Math.round((reference.product.compareAtPrice / reference.grams) * grams)
            : undefined;
        ...
        specifications.set("priceMethod", exact
        ...
        return {
            ...
            specifications,
            tags: [
                ...reference.product.tags.filter((tag) => !/^\d+\s*(?:kg|g|gm)$/i.test(tag)),
                quantity,
            ].slice(0, 30),
            isActive: true,
        };
