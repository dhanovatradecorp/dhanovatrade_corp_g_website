...
function workbookRows(buffer) {
    ...
    if (!catalogueSheet)
        throw new Error('The workbook does not contain a "Product Catalogue" sheet.');
    ...
    return rows.flatMap((row) => {
        if (Number(row["@_r"]) < 5)
            return [];
        const cells = new Map(asArray(row.c).map((cell) => [String(cell["@_r"]).replace(/\d+/g, ""), cellValue(cell)]));
...
function metaImage(html, pageUrl) {
    ...
    for (const tag of tags) {
        ...
        if (content)
            return new URL(decodeHtml(content), pageUrl).toString();
    }
    return "";
}
