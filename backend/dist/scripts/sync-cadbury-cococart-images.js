...
function packKey(value) {
    ...
    if (unit === "l" || unit.startsWith("litre"))
...
function isSafeNameMatch(catalogueName, sourceName) {
...
async function loadCocoCartProducts() {
    ...
    for (let page = 1; page <= 10; page += 1) {
        ...
        if (!response.ok)
            throw new Error(`CocoCart catalogue request failed with ${response.status}.`);
    ...
    return products.filter((product) => /cadbury|mondelez/i.test(`${product.vendor} ${product.title}`) &&
...
try {
    ...
    const result = matches.length
        ? await Product.bulkWrite(matches.map(({ product, source, score }) => ({
            updateOne: {
    ...
    matches
        .slice(0, 20)
        .forEach(({ product, source, score }) => console.log(`${Math.round(score * 100)}% | ${product.name} <- ${source.title}`));
}
finally {
