...
const localImage = (folder) => (itemIndex) => `/product-images/${folder}/${itemIndex % 10}.webp`;
const fallbackImage = (name) => () => `/product-images/catalogue-fallbacks/${name}.png`;
const departments = [
    {
...
const existingSlugs = new Set((await Product.find({
    slug: { $in: candidates.map((product) => product.slug) },
    isActive: true,
})
    .select("slug")
    .lean()).map((product) => product.slug));
const availableCandidates = candidates.filter((product) => !existingSlugs.has(product.slug));
const categoryCounts = new Map((await Product.aggregate([
    {
        $match: {
            isActive: true,
            category: {
...
const additions = departments.flatMap((department) => {
    ...
    if (available.length < needed)
        throw new Error(`${department.category} needs ${needed} products, but only ${available.length} expansion products are available.`);
...
if (totalAfterMinimums < targetTotal) {
    ...
    if (remaining.length < totalNeeded)
        throw new Error(`Catalogue needs ${totalNeeded} more products, but only ${remaining.length} expansion products are available.`);
...
for (let start = 0; start < additions.length; start += 500) {
    ...
    await Product.bulkWrite(batch.map((product) => ({
        updateOne: {
...
if (activeAfter < targetTotal || belowMinimum.length > 0)
    throw new Error(`Catalogue validation failed: ${activeAfter} active products; ${belowMinimum.length} categories below minimum.`);
