...
const requestedLimit = Number(process.argv
...
function packKey(value) {
    ...
    if (multi)
        return `${multi[1]}x${multi[2]}${multi[3]}`;
    ...
    if (countPack)
        return `${countPack[1]}pack`;
    return normalize(text);
}
function nameScore(catalogueName, sourceName) {
...
async function searchBigBasket(query, cookie) {
...
try {
    ...
    const updates = await mapWithConcurrency(targets, 1, async (product, index) => {
        ...
        if (debugMatches)
            console.log(`QUERY ${product.name} => ${results
                .slice(0, 5)
                .map((source) => `${source.brand?.name}|${source.desc}|${source.w}`)
                .join(" || ") || "no results"}`);
        const candidates = results
            .flatMap((source) => {
            const sourceImage = source.images?.[0]?.xxl ??
                source.images?.[0]?.xl ??
                source.images?.[0]?.l;
            if (!sourceImage || packKey(source.w) !== packKey(product.quantity))
                return [];
            const brandScore = canonicalBrand(source.brand?.name ?? "") ===
        ...
        if ((index + 1) % 50 === 0)
            console.log(`Checked ${index + 1}/${targets.length}; matches so far are being collected.`);
