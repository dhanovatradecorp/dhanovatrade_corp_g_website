...
function unitKey(amountText, unitText) {
...
function sourcePackKeys(value) {
    return new Set([...value.matchAll(/(\d+(?:\.\d+)?)\s*(ml|l|litres?|ltr)\b/gi)].map((match) => unitKey(match[1], match[2])));
}
function cataloguePackKey(value) {
...
function imageRepresentsPack(product, wantedPack) {
    const listedPacks = sourcePackKeys(product.product_display_size);
    if (listedPacks.size === 1)
        return listedPacks.has(wantedPack);
...
async function loadOfficialIceCreams() {
    ...
    if (!categoryResponse.ok)
        throw new Error(`Amul catalogue request failed with ${categoryResponse.status}.`);
    const payload = (await categoryResponse.json());
    return payload.product_category_data.sub_category.flatMap((subcategory) => subcategory.product
...
try {
    ...
    const cleared = await Product.updateMany({ "specifications.imageSource": SOURCE_LABEL, _id: { $nin: matchedIds } }, {
        $set: {
            images: [],
    ...
    const result = matches.length
        ? await Product.bulkWrite(matches.map(({ product, official }) => ({
            updateOne: {
