...
function normalizeRow(row) {
    const productName = row["Product Name"] || row["Product Name "] || "Untitled Product";
    ...
    const sellingPrice = parseCurrency(row["Price "] || row["Price"] || row["Price"]);
    ...
    const uniqueSuffix = productId || `${slugify(productName)}-${slugify(quantity)}`;
    return {
        name: productName,
        slug: `${slugify(productName)}-${slugify(uniqueSuffix)}`.replace(/-+/g, "-") || "product",
        description,
        brand,
        category,
        subcategory,
        ...
        stock: 40 + (Math.abs(productId.split("").reduce((sum, char) => sum + char.charCodeAt(0), 0)) % 180),
        quantity,
...
async function importCsvProducts(filePath) {
    ...
    console.log(`CSV import complete: ${totalInserted} inserted, ${totalUpdated} updated, ${activeTotal} active products total.`);
}
async function main() {
