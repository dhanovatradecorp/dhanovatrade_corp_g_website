...
const router = Router();
function paymentCredentials(provider = "razorpay") {
    if (provider === "zoho") {
        ...
        if (accessToken) {
            return { keyId: accessToken, keySecret: "", authType: "oauth" };
        }
        if (keyId && keySecret) {
            return { keyId, keySecret, authType: "basic" };
        }
...
function arePaymentCredentialsConfigured() {
    return Boolean(process.env.RAZORPAY_KEY_ID?.trim() && process.env.RAZORPAY_KEY_SECRET?.trim()) || Boolean(process.env.ZOHO_KEY_ID?.trim() && process.env.ZOHO_KEY_SECRET?.trim());
}
...
router.get("/config", (_request, response) => {
    response.json({
        ...
        providers: {
            razorpay: Boolean(process.env.RAZORPAY_KEY_ID?.trim() &&
                process.env.RAZORPAY_KEY_SECRET?.trim()),
            zoho: Boolean(process.env.ZOHO_KEY_ID?.trim() &&
                process.env.ZOHO_KEY_SECRET?.trim()),
        },
    });
});
router.post("/create-order", async (request, response) => {
    ...
    const gatewayEndpoint = provider === "zoho"
        ? "https://payments.zoho.com/api/v1/orders"
        : "https://api.razorpay.com/v1/orders";
    const gatewayHeaders = provider === "zoho"
        ? {
    ...
    const gatewayOrderId = gatewayOrder.id ?? gatewayOrder.order_id ?? gatewayOrder.orderId;
    const gatewayErrorMessage = gatewayOrder.error?.description ??
        gatewayOrder.message ??
        gatewayOrder.error_message ??
        (provider === "zoho" ? "Zoho rejected the payment order request" : "Unable to create payment order");
...
router.post("/verify", async (request, response) => {
    ...
    try {
        await reserveInventory(claimedOrder.items.map((item) => ({
    ...
    try {
        ...
        await releaseInventory(claimedOrder.items.map((item) => ({
            product: item.product,
            quantity: item.quantity,
        })));
        await Order.updateOne({ _id: claimedOrder._id, status: "processing" }, {
            $set: { status: "pending" },
