...
const normalizeTags = (value) => [...new Set(value.map((tag) => tag.trim().toLowerCase()).filter(Boolean))];
export const productInputSchema = z.object({
    ...
    images: z
        ...
        .string()
        .trim()
        .refine((value) => value.startsWith("/") || /^https?:\/\//i.test(value), "Image must be a local path beginning with / or a full http(s) URL"))
        .max(10)
        .default([])
