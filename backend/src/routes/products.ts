...
function normalizeProductPayload<T extends NormalizedProductPayload>(
  payload: T,
): T {
  ...
  if (next.specifications !== undefined) {
    const entries =
      next.specifications instanceof Map
        ? [...next.specifications.entries()]
        : Object.entries(next.specifications as Record<string, string>);

    next.specifications = Object.fromEntries(
      entries
  ...
  if (next.images !== undefined) {
    next.images = [
      ...new Set((next.images as string[]).filter(Boolean)),
    ] as T["images"];
  }
...
router.post(
  "/upload-image",
  requireAdmin,
  express.raw({ type: Object.keys(supportedImageTypes), limit: "5mb" }),
...
router.patch("/:id", requireAdmin, async (request, response) => {
  if (!isValidObjectId(request.params.id))
    throw new HttpError(400, "Invalid product ID");
  const updates = normalizeProductPayload(
